...
enum read_status from_bmp(FILE *in, struct image *img) {
  ...
  if (header.bfType != BMP_BITMAP_ID)
  ...
  for (uint32_t i = 0; i < header.biHeight; i++) {
    if (!fread((img->data) + i * header.biWidth, sizeof(struct pixel),
               header.biWidth, in))
...
#define BI_SIZE_IMAGE(height, width)                                           \
  (height) * (get_padding((width)) + (width) * sizeof(struct pixel))
#define BF_FILE_SIZE(height, width)                                            \
  (BF_OFF_BITS + BI_SIZE_IMAGE((height), (width)))
